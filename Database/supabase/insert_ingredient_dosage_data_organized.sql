-- =================================================================
-- INGREDIENT DOSAGE CONFIGURATIONS TABLE CREATION AND DATA
...
-- Create enum for dosage units
CREATE TYPE dosage_unit_enum AS ENUM (
    'mg',
    'g', 
    'mcg',
    'IU',
...
-- Create the ingredient dosage configurations table
CREATE TABLE public.ingredient_dosage_configs (
    id SERIAL PRIMARY KEY,
    ingredient_name VARCHAR(255) NOT NULL,
    ingredient_type ingredient_type_enum NOT NULL,
    category VARCHAR(100), -- 'fundamental', 'salt', 'vitamin', 'mineral', etc.
    dosage_unit dosage_unit_enum NOT NULL,
    
    -- Dosage recommendations
    min_daily_dosage DECIMAL(10,2),
    max_daily_dosage DECIMAL(10,2),
    dangerous_dosage DECIMAL(10,2),
    
    -- Additional information
    dosage_notes TEXT,
    cautions TEXT,
    precaution_people TEXT[], -- Array of medical conditions
    dosage_citation TEXT,
    cautions_citation TEXT,
    ...
    -- Constraints
    CONSTRAINT chk_dosage_values CHECK (
        (min_daily_dosage IS NULL OR min_daily_dosage >= 0) AND
        (max_daily_dosage IS NULL OR max_daily_dosage >= 0) AND
        (dangerous_dosage IS NULL OR dangerous_dosage >= 0)
    )
    
    -- No unique constraint to allow duplicates for different contexts
);

-- Create indexes for performance
CREATE INDEX idx_ingredient_dosage_configs_name ON public.ingredient_dosage_configs (ingredient_name);
CREATE INDEX idx_ingredient_dosage_configs_type ON public.ingredient_dosage_configs (ingredient_type);
CREATE INDEX idx_ingredient_dosage_configs_category ON public.ingredient_dosage_configs (category);
