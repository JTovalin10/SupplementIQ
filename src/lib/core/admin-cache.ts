...
class AdminCacheManager {
  ...
  private async refreshCache(): Promise<void> {
    try {
      ...
      console.log(`Admin cache refreshed: ${this.cache.admins.length} moderators/admins/owners cached`);
    } catch (error) {
  ...
  async addAdminToCache(adminData: { id: string; role: string }): Promise<void> {
    ...
    console.log(`✅ User added to cache: ${adminData.id} (${adminData.role})`);
  }
  ...
  async coldStart(): Promise<void> {
    ...
    console.log(`✅ Cold start complete: ${this.cache.admins.length} moderators/admins/owners loaded, owner: ${this.cache.ownerId}`);
  }
