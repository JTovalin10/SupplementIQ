...
export function OwnerDashboardProvider({
  ...
  const fetchDashboardData = useMemo(
    () => async () => {
      ...
      try {
        // Get session directly (removed cold start handler for speed)
        const {
          data: { session },
          error: sessionError,
        ...
        // Fetch pending submissions directly (removed cold start handler for speed)
        const pendingResponse = await fetch(
          "/api/admin/dashboard/pending-submissions?page=1&limit=10",
          {
            headers: {
        ...
        if (pendingResponse.ok) {
          ...
          throw new Error(
            `Failed to fetch pending submissions: ${pendingResponse.status}`,
          );
        }
  ...
  const handleApproveSubmission = useMemo(
  ...
  const handleRejectSubmission = useMemo(
    () => async (submissionId: string) => {
  ...
  const handlePromoteToModerator = useMemo(
    () => (targetUserId: string) => {
      console.log(
        `Owner ${user.id} promoting user ${targetUserId} to moderator`,
      );
  ...
  const handlePromoteToAdmin = useMemo(
    () => (targetUserId: string) => {
  ...
  const handleOverridePromote = useMemo(
    () => (targetUserId: string, role: string) => {
      console.log(
        `Owner ${user.id} override promoting user ${targetUserId} to ${role}`,
      );
