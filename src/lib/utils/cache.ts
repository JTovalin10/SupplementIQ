...
class SupplementIQCache {
  ...
  getStats() {
    ...
    return {
      ...
      hitRate: this.cache.size > 0 ? Math.round((totalHits / (totalHits + this.cache.size)) * 100) / 100 : 0
    };
  }
...
export const cacheKeys = {
  ...
  productSearch: (query: string, filters: any) => `search:${JSON.stringify({ query, filters })}`,
  user: (id: string) => `user:${id}`,
  contribution: (id: string) => `contribution:${id}`,
  ingredients: () => 'ingredients:all',
  transparencyRankings: (category?: string) => `rankings:transparency:${category || 'all'}`,
  costEfficiencyRankings: (category?: string) => `rankings:cost:${category || 'all'}`,
