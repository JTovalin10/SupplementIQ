 ...
 */
export async function verifyProductSubmission(
  ...
  try {
    console.log(
      `🔍 Verifying product submission: ${submission.name} (${submission.brand_name})`,
    );
    ...
    if (similarProducts.length > 0) {
      ...
      const highSimilarity = similarProducts.some(
        (p) => p.similarity_score > 0.9,
      );
      if (highSimilarity) {
        result.canSubmit = false;
    ...
    console.log(
      `✅ Verification completed: ${result.canSubmit ? "Can submit" : "Needs review"}`,
    );
    return result;
  } catch (error) {
...
async function findExactMatches(
  submission: ProductSubmission,
): Promise<VerificationResult["existingProducts"]> {
...
async function findSimilarProducts(
  ...
  const { data, error } = await supabase
    ...
    .select("id, name, brand_name, flavor, year")
    .or(
      `name.ilike.%${submission.name}%,brand_name.eq.${submission.brand_name.toLowerCase()}`,
    )
    .limit(20);
...
function calculateSimilarity(
  ...
  if (
    submission.brand_name.toLowerCase() === existing.brand_name.toLowerCase()
  ) {
    score += 1.0 * 0.3;
  }
  factors += 0.3;
 ...
 */
async function checkTrieMatches(
  submission: ProductSubmission,
...
function generateSuggestions(
  ...
  if (
    submission.flavor &&
    result.existingProducts.some((p) => p.flavor === submission.flavor)
  ) {
  ...
  if (!submission.year && submission.category === "pre_workout") {
    result.suggestions.push(
      "Consider adding the year for pre-workout formulas",
    );
  }
}
...
export async function submitProductForApproval(
  ...
  try {
    ...
    console.log(
      `✅ Product submitted for approval: ${submission.name} (${submission.brand_name})`,
    );
    return {
      success: true,
