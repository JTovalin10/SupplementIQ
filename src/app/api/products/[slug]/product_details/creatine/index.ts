...
export async function fetchCreatineDetails(
  ...
  console.log(
    `Fetching creatine details for ${isPending ? "pending" : "approved"} product ID:`,
    productId,
  );
  ...
  if ((error || !data) && !isPending) {
    console.log(
      `No creatine details found with product_id=${productId}, looking for matching pending_products entry`,
    );
    ...
    if (productData) {
      ...
      if (pendingData) {
        console.log(
          `Found pending_products entry with id=${pendingData.id}, querying creatine_details`,
        );
  ...
  // Transform the data to include creatine dosage in mg
  // Use creatine_dosage_mg if available, otherwise use serving_size_g (which is the creatine amount per serving)
  let creatineDosageMg = 0;
  if (data.creatine_dosage_mg && data.creatine_dosage_mg > 0) {
    creatineDosageMg = data.creatine_dosage_mg;
  } else if (data.serving_size_g && data.serving_size_g > 0) {
    // serving_size_g is the actual creatine amount per serving
    creatineDosageMg = Math.round(parseFloat(data.serving_size_g) * 1000); // Convert g to mg
  } else {
    // Fallback to recommended daily dose if no actual dosage data
    creatineDosageMg = data.creatine_types?.recommended_daily_dose_g
      ? Math.round(data.creatine_types.recommended_daily_dose_g * 1000)
