    ...
    for (const product of products) {
      try {
        ...
        if (detailsError && detailsError.code !== "PGRST116") {
          errors.push(
            `Error fetching details for product ${product.id}: ${detailsError.message}`,
          );
          continue;
        }

        // Calculate ratings
        const dosageRating = await calculateDosageRating(
          product.category,
          product.id,
          details,
        );
        const dangerRating = await calculateDangerRating(
          product.category,
          product.id,
          details,
        );
        ...
        if (updateError) {
          errors.push(
            `Error updating product ${product.id}: ${updateError.message}`,
          );
        } else {
          updatedCount++;
        }
 ...
 */
async function calculateDosageRating(
  category: string,
  productId: number,
  details: any,
): Promise<number> {
 ...
 */
async function calculateDangerRating(
  category: string,
  productId: number,
  details: any,
): Promise<number> {
  ...
  const { ingredientDosageCache } = await import(
    "@/lib/cache/ingredient-dosage-cache"
  );
  // Get ingredient configs for the category and convert to object
  const categoryConfigsMap =
    await ingredientDosageCache.getIngredientConfigsForCategory(category);
  ...
  for (const [ingredientName, actualDosage] of Object.entries(details)) {
    if (!actualDosage || typeof actualDosage !== "number" || actualDosage <= 0)
      continue;
    ...
    if (
      ingredientConfig.precaution_people &&
      ingredientConfig.precaution_people.length > 0
    ) {
