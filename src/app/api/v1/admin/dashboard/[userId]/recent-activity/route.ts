...
export async function GET(
  ...
  try {
    ...
    const { data: requestingUserProfile, error: requestingUserError } = await supabase
      .from('users')
      .select('role')
      .eq('id', user.id)
      .single();
    ...
    if (user.id !== userId && requestingUserProfile.role !== 'owner') {
      return NextResponse.json({ error: 'Can only access your own dashboard data' }, { status: 403 });
    }
    ...
    if (!['admin', 'owner'].includes(targetUserProfile.role)) {
      return NextResponse.json({ error: 'Target user does not have dashboard access' }, { status: 403 });
    }
