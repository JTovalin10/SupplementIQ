  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.name || !formData.brand || !formData.category) {
        setSubmitStatus("error");
        setSubmitError(
          "Please fill in all required fields (Product Name, Brand, and Category)",
        );
        setIsSubmitting(false);
        return;
      }
      ...
      const submissionData: any = {
        ...
        ...(canSubmitImageUrl &&
          processedFormData.imageUrl && {
            image_url: processedFormData.imageUrl,
          }),
        job_type: "add",
      ...
      if (
        processedFormData.servings_per_container &&
        processedFormData.servings_per_container !== "not_in_product"
      ) {
        submissionData.servings_per_container = parseFloat(
          processedFormData.servings_per_container,
        );
      }

      if (
        processedFormData.price &&
        processedFormData.price !== "not_in_product"
      ) {
        submissionData.price = parseFloat(processedFormData.price);
      }

      if (
        processedFormData.serving_size_g &&
        processedFormData.serving_size_g !== "not_in_product"
      ) {
        submissionData.serving_size_g = parseFloat(
          processedFormData.serving_size_g,
        );
      }

      // Add all ingredient fields from the form
      // Get all ingredient fields for the current category
      const categoryIngredientFields = getCategoryIngredients(
        formData.category,
      );

      // Process each ingredient field and add to submission
      categoryIngredientFields.forEach((ingredient) => {
        const fieldName = ingredient.name;
        const fieldValue = processedFormData[fieldName];

        // Skip if not set or if it's a basic info field we already handled
        if (fieldValue === undefined || fieldValue === null) return;

        // Convert form values to database values
        if (fieldValue === "" || fieldValue === "not_in_product") {
          submissionData[fieldName] = 0; // Not in product
        } else if (fieldValue === "not_specified") {
          submissionData[fieldName] = -1; // Blend/unknown
        } else {
          // Try to parse as number
          const numValue = parseFloat(fieldValue);
          if (!isNaN(numValue)) {
            submissionData[fieldName] = numValue;
          }
        }
      });

      // Submit to pending products API
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Card padding="lg">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">
              {formData.category
                ? `${formData.category.charAt(0).toUpperCase() + formData.category.slice(1).replace("-", " ")} Information`
            ...
            {formData.category ? (
              ...
              <div className="space-y-4">
                <p className="text-gray-600">
                  Select a category above to see ingredient fields for that
                  product type.
                </p>
              </div>
            )}
