...
export default function NewProductBasicInfo({
  ...
  // Set category search term when value changes
  useEffect(() => {
    const currentValue = formData.category || '';
    if (currentValue) {
      const selectedCategory = categories.find(cat => cat === currentValue);
      if (selectedCategory) {
        setCategorySearchTerm(selectedCategory.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase()));
      }
    } else {
  ...
  const handleCategorySelect = (category: string) => {
    ...
    setCategorySearchTerm(category.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase()));
    setIsCategoryOpen(false);
  };
  ...
  const filteredCategories = categories.filter(category =>
    ...
    category.replace('-', ' ').toLowerCase().includes(categorySearchTerm.toLowerCase())
  );
                    ...
                    <p>
                      ...
                      {user && (
                        <span className="block mt-1">
                          Current: {user.reputation_points} points, {user.role} role
                        </span>
                      )}
                    </p>
                  </div>
