...
export default function ProductModeration() {
  ...
  const handleApproval = async (type: 'submission' | 'edit', id: string, action: 'approve' | 'reject') => {
    try {
  ...
  const getRoleBadgeColor = (role: string) => {
    switch (role) {
      ...
      case 'trusted_editor': return 'bg-green-100 text-green-800';
    ...
    return (
      <Card className="p-6">
        <div className="text-center">
          ...
          <p className="text-gray-600">You need Trusted Editor role or higher to view pending products.</p>
        </div>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Review and approve product submissions and edits</p>
        </div>
        <div className="flex items-center space-x-4">
          <Badge className={getRoleBadgeColor(userRole.role)}>
            {userRole.role.replace('_', ' ').toUpperCase()}
          </Badge>
          <span className="text-sm text-gray-600">{userRole.contributions} contributions</span>
        </div>
      </div>
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className="text-sm text-gray-600">
                      {new Date(activeTab === 'submissions' ? item.submittedAt : item.editedAt).toLocaleDateString()}
                    </span>
                  </div>
                  ...
                  <p className="text-sm text-gray-600">
                    By: {activeTab === 'submissions' ? item.submittedBy : item.editedBy}
                  </p>
                </div>
            ...
            <Card className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  {activeTab === 'submissions' ? 'Product Submission' : 'Product Edit'}
                </h3>
              ...
              <div className="space-y-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">
                    {activeTab === 'submissions' 
                      ? selectedItem.productDetails?.description || 'No description'
                      : selectedItem.editDetails?.description || 'No description'
                    }
                  </p>
                </div>
              ...
              <div className="flex space-x-3">
                <Button
                  onClick={() => handleApproval(activeTab, selectedItem.id, 'approve')}
                ...
                <Button
                  variant="destructive"
                  onClick={() => handleApproval(activeTab, selectedItem.id, 'reject')}
                  disabled={!userRole?.permissions.canApproveSubmissions}
                  className="flex items-center space-x-2"
                >
