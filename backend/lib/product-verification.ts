...
import { fileAutocompleteService } from './services/file-autocomplete';
...
/**
 * Check Trie for autocomplete matches
 */
async function checkTrieMatches(submission: ProductSubmission, result: VerificationResult): Promise<void> {
    // Check if product name exists in Trie
    const productMatches = fileAutocompleteService.searchProducts(submission.name, 5);
    if (productMatches.length > 0) {
        result.warnings.push(`Product name "${submission.name}" found in autocomplete`);
    }

    // Check if brand exists in Trie
    const brandMatches = fileAutocompleteService.searchBrands(submission.brand_name, 5);
    if (brandMatches.length > 0) {
        result.warnings.push(`Brand "${submission.brand_name}" found in autocomplete`);
    }

    // Check if flavor exists in Trie
    if (submission.flavor) {
        const flavorMatches = fileAutocompleteService.searchFlavors(submission.flavor, 5);
        if (flavorMatches.length > 0) {
            result.warnings.push(`Flavor "${submission.flavor}" found in autocomplete`);
        }
    }
}
