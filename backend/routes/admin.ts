...
/**
 * Admin routes for managing autocomplete data
 * These should be protected with admin authentication
 */

/**
 * @route POST /api/v1/admin/update-autocomplete
 * @desc Trigger daily autocomplete update
 * @access Admin only
 * @returns 200 - Update completed successfully
 * @returns 409 - Update already in progress
 * @returns 500 - Update failed
 */
router.post('/update-autocomplete', adminAuth, async (req: Request, res: Response) => {
  try {
    ...
    dailyUpdateService.performDailyUpdate()
      .then(() => {
        console.log('✅ Manual autocomplete update completed');
      })
      .catch((error) => {
        console.error('❌ Manual autocomplete update failed:', error);
      });

    res.json({
      success: true,
      message: 'Autocomplete update started',
      status: 'running'
    });

  } catch (error) {
    console.error('Failed to start autocomplete update:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error',
      message: 'Failed to start autocomplete update'
    });
  }
});

/**
 * @route GET /api/v1/admin/autocomplete-status
 * @desc Get autocomplete service status
 * @access Admin only
 * @returns 200 - Status information
 */
router.get('/autocomplete-status', adminAuth, async (req: Request, res: Response) => {
  try {
    const status = dailyUpdateService.getUpdateStatus();
    ...
    console.error('Failed to get autocomplete status:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error',
      message: 'Failed to get autocomplete status'
    });
  }
});
 ...
 * @desc Request an autocomplete update (requires admin voting or owner approval)
 * @access Admin only
...
router.get('/autocomplete-stats', adminAuth, async (req: Request, res: Response) => {
  try {
    ...
    const stats = fileAutocompleteService.getStats();
    const status = dailyUpdateService.getUpdateStatus();
